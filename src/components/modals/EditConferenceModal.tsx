...
const EditConferenceModal: React.FC<EditConferenceModalProps> = ({ isOpen, onClose, conference }) => {
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-lg mx-4 animate-scale-in max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-foreground font-display">Edit Conference</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
          <FloatInput label="Conference Name" defaultValue={conference.name} required />
          <FloatInput label="Bishop / Presiding Prelate" defaultValue={conference.bishop} required />
          <FloatInput label="Headquarters Address" defaultValue={conference.address} />
          <div className="grid grid-cols-2 gap-4">
            <FloatInput label="Phone Number" defaultValue={conference.phone} />
            <FloatInput label="Email Address" type="email" defaultValue={conference.email} />
          </div>
          <FloatInput label="Region" defaultValue={conference.region} />
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Status</label>
            <select 
              defaultValue={conference.status || 'active'}
